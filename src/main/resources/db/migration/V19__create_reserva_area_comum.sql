...
--2. Criação da tabela de status
CREATE TABLE status_reserva (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    nome VARCHAR(255) UNIQUE NOT NULL
);

--2.1 Adicionar estados obrigatórios
...
CREATE TABLE reserva (
    ...
    CONSTRAINT fk_reserva_area_comum FOREIGN KEY (area_comum_id) REFERENCES area_comum (id),
    ...
    CONSTRAINT fk_reserva_status FOREIGN KEY (status_id) REFERENCES status_reserva (id),
...
CREATE INDEX idx_reserva_status_id ON reserva (status_id);
